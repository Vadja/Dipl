...
import org.iit.dr.loaders.ThemeGraduationProjectLoader;
...
//    bulkPopup.add( item );
  ...
  protected void generateTheme(String graduateWorkId){
	    ...
	    if( graduateWork != null )
	    {
	      ...
	      String stGroup = checkData(OrganizationUnitService.getOrganizationUnit(student.getGroup()).getName());
	      String stLastName= checkData(student.getLastName());
		  String stFirstName= checkData(student.getFirstName());
		  String stPatronymicName = checkData(student.getPatronymicName());
		  ...
		  String mgDegree = checkData(manger.getDegree()!=null?manger.getDegree().getRusName():"");
		  String mgRank = checkData(manger.getRank()!=null?manger.getRank().getRusName():"");
		  String mgAddress = checkData(manger.getAddress());
		  String mgHomePhone = checkData(manger.getHomePhone());
		  String mgPosition = checkData(manger.getPosition()!=null?manger.getPosition().getRusName():"");
  ...
  protected void generateComponents()
  {
    ...
    JButton loadButton = new JButton( "Загрузить список дипломников" );
...
//          fileChooser.setFileFilter(filter);
          ...
          if (result == JFileChooser.APPROVE_OPTION){
          	ThemeGraduationProjectLoader loader = new ThemeGraduationProjectLoader();
          	loader.loadData(fileChooser.getSelectedFile().getAbsolutePath(), model);
//          	groupTreeSimplePanel.updateData();
    ...
    getContentPane().add( createVPane( 0, createNorthButtonPane( 5,loadButton, addButton, removeButton ), jFilterPanel ),
      BorderLayout.NORTH );
