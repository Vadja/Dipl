 ...
 */
public class ExternalStafferFrame extends
		JInternalFrameExt<ExternalSecondJobStaffer> {

	private boolean isNewStaffer;

	ExternalSecondJobStaffer staffer;
	...
	protected void init() {
		...
		setTitle("Форма редактирования: Внешний совместитель");
	...
	protected void generateComponents() {
		...
		JPanel leftJPane = createVPane(5,
				createField(lastNameField, "Фамилия: "),
				createField(firstNameField, "Имя: "),
				createField(patrNameField, "Отчество: "),
				createField(dogovor),
				createField(contract)
				);

		JPanel rightJPane = createVPane(
				5,
				createField(jDateRange, "Срок: "),
				createField(rankField, "Звание: "),
				createField(degreeField, "Степень: "),
				createField(originalPositionField, "Должность (осн.): "),
				createField( secondJobPositionField, "Должность по совм.: " ));
		...
		JPanel passportPane = createVPane(
				5,
				createField(passportNumberField, "№ паспорта: "),
				createField(passportPrivateNumberField, "Личный номер: "),
				createField(passportDateIssueField, "Дата выдачи: "),
				createField(passportPlaceIssueField,
						"Место выдачи (структура): "),
		...
		rateTable.setDefaultEditor(RateType.class, new DefaultCellEditor(
				rateTypeBox));

		JScrollPane jScrollPane = new JScrollPane(rateTable,
		...
		getContentPane().add(
				createNorthButtonPane(10, okButton, applyButton, cancelButton),
				BorderLayout.SOUTH);
	}
	...
	@Override
	public boolean showFrame(Object parent, ExternalSecondJobStaffer staffer) {
		...
		if (staffer != null) {
			...
			this.staffer = new ExternalSecondJobStaffer();
		...
		if (this.staffer.getPassport() != null) {
			passportDateIssueField.setDate(this.staffer.getPassport()
					.getDateIssue());
			passportNumberField.setText(this.staffer.getPassport().getNumber());
			passportPlaceIssueField.setText(this.staffer.getPassport()
					.getPlaceIssue());
			passportPrivateNumberField.setText(this.staffer.getPassport()
					.getPrivateNumber());
			passportInsuranceNumberField.setText(this.staffer.getPassport()
					.getInsuranceNumber());
		}
	...
	//
	// if (staffer != null && staffer.getId() != null &&
	// staffer.getId().length()!=0){
	//
	// }
	// }
}